...
const CompanionComponent = ({companionId,subject,topic,name,userName,userImage,style,voice}:CompanionComponentProps) => {
    ...
    const toggleMicrophone = () => {
        if (vapi && typeof vapi.setMuted === 'function' && typeof vapi.isMuted === 'function') {
          const currentMuted = vapi.isMuted();
          vapi.setMuted(!currentMuted);
          setisMuted(!currentMuted);
        } else {
          console.warn("⚠️ vapi.setMuted or vapi.isMuted is not available in this version of the SDK.");
        }
      };
...
vapi.stop()
    ...
    return (
        ...
        <section className='transcript'>
            <div className='transcript-message no-scroll-bar'>
                {messages.map((message,index)=>{
                    if(message.role=='assistant'){
                        return(
                            <p key={index} className='max-sm:text-sm text-primary'>{name.split('')[0].replace('[],.:','')}:{message.content}</p>
                        )
                    }else{
                        return(
